...
function ga_dash_content() {
	...
	$metrics = 'ga:'.$query;
	$dimensions = 'ga:year,ga:month,ga:day';
	$opts = array('dimensions' => $dimensions);
	global $user_ID;
	get_currentuserinfo();
	if(current_user_can('channel')){
		$channel = new WP_Query(array('post_type'=>'channel', 'author'=>$user_ID, 'posts_per_page'=>1));
		if($channel->have_posts()){
			$opts['filters'] = 'ga:pagePath=@/channel/'.$channel->posts[0]->post_name;
		}
		wp_reset_postdata();
	}

	try{
		$serial='gadash_qr2'.str_replace(array('ga:',',','-',date('Y')),"",$user_ID.$projectId.$from.$to.$metrics);
		$transient = get_transient($serial);
		if ( empty( $transient ) ){
			$data = $service->data_ga->get('ga:'.$projectId, $from, $to, $metrics, $opts);
			set_transient( $serial, $data, get_option('ga_dash_cachetime') );
		}else{
			$data = $transient;	 	
		}	
	...
	try{
		$serial='gadash_qr3'.str_replace(array('ga:',',','-',date('Y')),"",$user_ID.$projectId.$from.$to);
		$transient = get_transient($serial);
		if ( empty( $transient ) ){
			$data = $service->data_ga->get('ga:'.$projectId, $from, $to, $metrics, $opts);
			set_transient( $serial, $data, get_option('ga_dash_cachetime') ); 
