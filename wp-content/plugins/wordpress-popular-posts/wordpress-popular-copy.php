	...
	class WordpressPopularPosts extends WP_Widget {
		...
		private function __get_img($p, $id = null, $url = null, $dim = array(80, 80), $crop = true, $source = "featured", $title) {
			...
			if ( $id ) {
				...
				// added as a work-around - spark sai
				
				$file_path = str_replace('citylinktv.sparklabs.in/wp-content/uploads', 'uploads.citylinktv.com', $file_path);
			...
			if ( file_exists(trailingslashit($this->uploads_dir['basedir']) . $p->id . '-' . $source . '-' . $dim[0] . 'x' . $dim[1] . '.' . $file_info['extension']) ) {
				...
				//return $this->_render_image($file_path, $dim, 'wpp-thumbnail wpp_cached_thumb wpp_' . $source, $title );
			...
			if ($source == "featured") {
				...
				if ($thumbnail_id) {
					...
					//return get_attached_file($thumbnail_id);
					 $src = wp_get_attachment_image_src( get_post_thumbnail_id($id), array( 300,300 ), false, '' ); 
					//echo $src[0];
					return $src[0];
				}
